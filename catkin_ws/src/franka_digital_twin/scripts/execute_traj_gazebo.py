#!/usr/bin/env python3
import rospy, yaml
from trajectory_msgs.msg import JointTrajectory, JointTrajectoryPoint
from control_msgs.msg import FollowJointTrajectoryAction, FollowJointTrajectoryGoal
import actionlib

TIME_SCALE = 10.0
SMOOTH_START = 2.0
EPS = 0.001 

if __name__ == "__main__":
    rospy.init_node("execute_traj_gazebo")
    
    yaml_path = "/home/yuan8868/catkin_ws/src/franka_digital_twin/traj/panda_traj.yaml"
    rospy.loginfo(f"Loading trajectory from: {yaml_path}")
    with open(yaml_path, 'r') as f:
        data = yaml.safe_load(f)

    traj = JointTrajectory()
    traj.joint_names = data['joint_names']
    traj.points = []

    
    first_point = data['points'][0]
    pt0 = JointTrajectoryPoint()
    pt0.positions = first_point['positions']
    pt0.velocities = [0.0]*7
    pt0.time_from_start = rospy.Duration(0.0)
    traj.points.append(pt0)

    
    pt1 = JointTrajectoryPoint()
    pt1.positions = first_point['positions']
    pt1.velocities = [0.0]*7
    pt1.time_from_start = rospy.Duration(SMOOTH_START)
    traj.points.append(pt1)

    last_time = SMOOTH_START
    for p in data['points']:
        pt = JointTrajectoryPoint()
        pt.positions = p['positions']
        pt.velocities = p.get('velocities', [0.0]*7)
        secs = p['time_from_start']['secs']
        nsecs = p['time_from_start']['nsecs']
        t = rospy.Duration(secs, nsecs).to_sec() * TIME_SCALE + SMOOTH_START
        if t <= last_time:
            t = last_time + EPS
        pt.time_from_start = rospy.Duration.from_sec(t)
        traj.points.append(pt)
        last_time = t

    rospy.loginfo(f"Trajectory loaded: {len(traj.points)} points")
    rospy.loginfo(f"First point: {traj.points[0].positions}")
    rospy.loginfo(f"Last point: {traj.points[-1].positions}")

    client = actionlib.SimpleActionClient(
        '/position_joint_trajectory_controller/follow_joint_trajectory',
        FollowJointTrajectoryAction
    )
    rospy.loginfo("Waiting for Gazebo Action Server...")
    client.wait_for_server()
    rospy.loginfo(" Gazebo Action Server connected.")

    goal = FollowJointTrajectoryGoal()
    goal.trajectory = traj

    rospy.loginfo("Sending trajectory to Gazebo...")
    client.send_goal(goal)
    client.wait_for_result()

    result = client.get_result()
    rospy.loginfo(f"Gazebo Execution finished. Result: {result}")

